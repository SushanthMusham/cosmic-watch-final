...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await axios.post("https://cosmic-watch-final.vercel.app/api/auth/register", {
        username,
        email,
        password,
      });
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-black text-white relative overflow-hidden">
      ...
      <div className="z-10 w-full max-w-md p-8 bg-space-900/60 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl animate-in fade-in slide-in-from-bottom-10 duration-1000">
        <div className="flex justify-center mb-6">
          <div className="p-4 bg-accent-cyan/10 rounded-full border border-accent-cyan/50 shadow-[0_0_15px_rgba(0,240,255,0.3)]">
            <UserPlus className="w-10 h-10 text-accent-cyan" />
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative group">
            <User className="absolute left-3 top-3.5 w-5 h-5 text-gray-500 group-focus-within:text-accent-cyan transition-colors" />
            <input
              type="text"
          ...
          <button
            type="submit"
            className="w-full mt-2 bg-accent-cyan text-black font-bold py-3.5 rounded-lg hover:bg-cyan-300 transition-all shadow-[0_0_20px_rgba(0,240,255,0.4)] hover:shadow-[0_0_30px_rgba(0,240,255,0.6)] transform hover:-translate-y-0.5"
          >
            AUTHORIZE & LAUNCH
          </button>
        </form>
        ...
        <div className="mt-8 text-center text-xs text-gray-500">
          Already authorized?{" "}
          <Link href="/login" className="text-accent-cyan hover:underline hover:text-cyan-300 transition-colors">
            Login here
          </Link>
        </div>
      </div>
